...
async function main() {
  ...
  try {
    ...
    if (target) { const pg = pageOf(target); if (pg) { figma.currentPage = pg; page = pg; } if ('x' in target) { ox = target.x; oy = target.y + ('height' in target ? target.height : 0) + 120; } }
  } catch (e) { /* default to current page origin */ }

  const title = T('VSee Patient Portal — v3 flow (all screens)', { size: 40, weight: 'bold', ls: -2 });
  title.x = ox; title.y = oy; page.appendChild(title);
  const sub = T('Rebuilt as native, editable Figma layers from https://kientr.github.io/vsee-portal/v3/', { size: 16, color: C.textSecondary });
  sub.x = ox; sub.y = oy + 52; page.appendChild(sub);

  buildLogin(); buildDashboard(); buildVisits(); buildVisitDetail(); buildEConsult();
  buildSPProvider(); buildSPType(); buildSPSlot(); buildSPReview(); buildConfirm();
  buildRequests(); buildRequestDetail(); buildNewRequest(); buildMessages(); buildForms();
  buildIntake(); buildRecords(); buildProfile(); buildSettings(); buildInCall();

  const cols = 4, gapX = 100, gapY = 110, startY = oy + 110;
  const yCol = new Array(cols).fill(startY);
  SCREENS.forEach((s, i) => { const c = i % cols; s.x = ox + c * (SCREEN_W + gapX); s.y = yCol[c]; page.appendChild(s); yCol[c] += s.height + gapY; });

  figma.currentPage.selection = SCREENS;
  figma.viewport.scrollAndZoomIntoView(SCREENS.concat([title]));
  figma.closePlugin('Done — added ' + SCREENS.length + ' v3 screens as editable layers.');
}
